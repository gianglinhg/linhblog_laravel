...
class FixxTinController extends Controller
{
    public function index(){
        $title = 'Quản trị tin';
        return view('tin.home', compact('title'));
    }
    public function list(){
        $title = 'Danh sách tin tức';
        $data = Tin::all();
        return view('tin.danhsach', compact('data','title'));
    }
    public function add(){
        $title = 'Thêm tin tức';
        return view('tin.them',compact('title'));
    }
    ...
    public function update($id){
        ...
        $t = Tin::find($id);
        if($t == null) return redirect('/tin/thongbao');
        return view('tin/capnhattin',['t'=>$t],compact('title'));
    }
