...
class CommentController extends Controller
{
    ...
    public function deleteCmt($id){
        if(!empty($id)){
            $deleteStatus = Comment::where('id',$id)->delete();
            if($deleteStatus) return back()->with('success','Đã xóa bình luận');
            else $error = 'Xóa tin không thành công';
        }else
    }
    return back()->with('error','Tin cần xóa không tồn tại');
}
