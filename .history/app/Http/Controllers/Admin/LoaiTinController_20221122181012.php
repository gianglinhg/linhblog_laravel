...
class LoaiTinController extends Controller
{
    ...
    public function __construct(){
        $this->tin = new Loaitin();
    }
    public function index(){
        $this->data['title'] = 'Thực hành tương tác Database';

        // $tin = DB::select('select * from loaitin where id=:id',[
        //     'id' => 12
        // ]);
    ...
    public function list(){
        ...
        // $tinList = $this->tin->get5Tin();
        return view('admin.loaitin.list', compact('title'));
    }
    public function add(){
        $title = "Thêm loại tin";
    ...
    public function edit($id=0, Request $request){
        ...
        if(!empty($id)){
            ...
            if(!empty($tinDetail[0])){
                ...
                return to_route('list')->with('msg', 'Tin tức không tồn tại');
            }
        }else{
            return to_route('list')->with('msg', 'Liên kết không tồn tại');
        }
        return view('edit', compact('title','tinDetail'));
    }

    public function postEdit(Request $request){
        $id = session('id');
        if(empty($id)) return to_route('list')->with('msg','Liên kết không tồn tại');
        $request->validate([
            'name' => 'required | min:5',
    ...
    public function delete($id){
        ...
        return to_route('list')->with('msg', $msg);
    }
}
