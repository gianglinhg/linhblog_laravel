...
class FixxTinController extends Controller
{
    ...
    public function search(){
        ...
        if(!($keyword == "")){
            ...
            // dd($title);
            $searchTin = Tin::where('tieuDe','LIKE', '%' . $keyword .'%')->paginate(config('tin.paginate'));
            return view('admin.tin.search', compact('searchTin','title'));
        }else{
            $msgSearch = "Không thể tìm kiếm";
            return to_route('tin_list', compact('msgSearch'));
        }
    }
}
