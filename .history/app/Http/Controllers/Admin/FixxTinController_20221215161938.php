...
class FixxTinController extends Controller
{
    ...
    public function list(Request $request){
        ...
        // dd($sortType);
        // if($sortType == 'asc') $sort = 'asc';
        $sortType == 'desc' ? $sort = 'asc' : $sort = 'desc';
        $data = $data->orderBy('tin.id',$sort)->paginate(config('tin.paginate'));
        $allLoaiTin = DB::table('loaitin')->get();
        return view('admin.tin.list', compact('data','title','mainTitle','allLoaiTin','sort'));
    }
    public function add(){
        $mainTitle = 'Tin tức';
        $title = 'Thêm tin tức';
