    ...
    <tbody>
        <tr>
            ...
            <td scope="col" class="text-center">
                <img src="/template/{{ $item->avatar }}" width="50px" class="rounded-circle" style="object-fit: cover"
                    </td>
            <td scope="col">{{ $item->name }}</td>
            <td scope="col">{{ $item->email }}</td>
            <td scope="col">{{ date('H:i - d/m/Y', strtotime($item->created_at)) }}</td>
            <td scope="col" width="10%" class="text-center">
                <a onclick="return confirm('Bạn có chắc chắn xóa ?')"
                    href="{{ route('admin.users.delete', ['id' => $item->id]) }}" class="btn btn-warning mt-1">Hủy</a>
            </td>
        </tr>
    </tbody>
    @empty
